% Transmissão digital - Modulação BPSK - Canal AWGN
% Código RS(31,27) - Soft-Decoded-> Algoritmo de Chase 
clear all;
close all;
clc;
% --------- %
L = 10^6;           % Palavras de k simbolos que serão geradas. 
% --------- %
    ...
    for j = 1 : L
        ...
        for ii = 1 : n
            for jj = 1 : m
                ...
                msg_ruido(ii,jj) = modulacao(ii,jj) + ruido(ii,jj);
                
                if  (real(msg_ruido(ii,jj)) > 0)        % Hard decision
                    msg_dem(ii,jj) = 0;
                else
                    msg_dem(ii,jj) = 1;
                end
